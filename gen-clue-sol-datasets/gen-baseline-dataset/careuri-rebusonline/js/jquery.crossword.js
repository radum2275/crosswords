...
*/
(function ($) {
	$.fn.crossword = function (entryData, title, info) {
		/*
		...
		*/
		...
		this.before("<br><br><h1><font size='14'>" + title + "</font></h1><br><br><br>")
		// append clues markup after puzzle wrapper div
		// This should be moved into a configuration object
		cluesText = '<table class="clues" id="puzzle-clues"><tr><td valign="top"><h1>Orizontal</h1>' +
			'<ul id="across" style="list-style-type:none; margin:0; padding:0"></ul></td>' +
			'<td valign="top"><h1>Vertical</h1>' +
			'<ul id="down" style="list-style-type:none; margin:0; padding:0">' +
			'</ul></td></tr></table>'
		this.after(cluesText);
		// initialize some variables
		var tbl = ['<table id="puzzle" class="grid">'],
			puzzEl = this,
			clues = $('#puzzle-clues'),
			clueLiEls,
			coords,
		...
		otherInfo = '<div id="other-info">' + info.replace(";", "<br>") + '</div>'
		...
		var puzInit = {
			init: function () {
				...
				// DELETE FOR BG
				puzzEl.append("<table><tr><td>");
				puzInit.buildTable();
				puzInit.buildEntries();
				puzzEl.append("</td></tr><tr><td>")
				puzzEl.append('<br><br>' + otherInfo)
				puzzEl.append("</td></tr></table>")
			},
				...
				*/
				for (var i = 0, p = entryCount; i < p; ++i) {
					...
					for (var x = 0, j = puzz.data[i].answer.length; x < j; ++x) {
						entries[i].push(x);
						coords = puzz.data[i].orientation === 'across' ?
					...
					// AB: Here they create the list of clues
			...
			buildTable: function () {
				for (var r = 1; r <= rows; ++r) {
					...
					for (var c = 1; c <= cols; ++c) {
						tbl.push('<td data-coords="' + c + ',' + r + '"></td>');
					};
					tbl.push("</tr>");
				};
				tbl.push("</table>");
			...
			*/
			buildEntries: function () {
				...
				// Put numbers of the first row and column
				for (var i = 1, p = entryCount; i <= p; ++i) {
					$groupedLights = $('.entry-' + i);
					if (!$('.entry-' + i + ':eq(0) span').length) {
						if (entries[i - 1][0].split(',')[1] == 1)
							$groupedLights.eq(0)
								.append('<span STYLE="font-size: 12pt; margin-top: 4pt">' +
									entries[i - 1][0].split(',')[0] + '</span>');
						if (entries[i - 1][0].split(',')[0] == 1)
							$groupedLights.eq(0)
								.append('<span STYLE="font-size: 12pt; margin-top: 4pt">' +
									entries[i - 1][0].split(',')[1] + '</span>');
					}
				}
				util.highlightEntry();
				util.highlightClue();
			...
			checkAnswer: function (e) {
				...
				if (valToCheck === currVal) {
					$('.active')
						.addClass('done')
						.removeClass('active');
					// Uncomment this to mark clues done with a strike line
			...
			*/
			markCorrectAnswers: function () {
				...
				for (var pos = 0; pos < puzz.data.length; pos++) {
					valToCheck = puzz.data[pos].answer.toLowerCase();
					currVal = $('.position-' + pos + ' input')
						.map(function () {
							return $(this)
								.val()
								.toLowerCase();
